 ...
 * @version    1.0.0
 */
class esWidgetFormDateJQuery extends sfWidgetFormInput
{
  /**
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    $visibleValue = (empty($value)? '' : date('D, d-M-Y', strtotime($value)));
    $fieldValue   = (empty($value)? '' : date('Y/m/d',    strtotime($value)));

    $attributes['disabled'] = 1;

    $html = parent::render('datepicker_'.$name, $visibleValue, $attributes, $errors).
            $this->renderTag('input', array('type' => 'hidden', 'name' => $name, 'value' => $fieldValue));

    //file_put_contents('log.txt', $html);
    $id = $this->generateId($name);
    $alt_id = $this->generateId('datepicker_'.$name);
...
<script type="text/javascript">
    ...
    params.buttonImage = '/images/calendar.gif';
...
<script type="text/javascript">
	$(function() {
    var params = {
    onSelect: function(dateText, instance) {console.log(dateText); console.log(this.value); this.value = dateText;},
    altField: '#$alt_id',
    changeMonth : $cm,
    changeYear : $cy,
    numberOfMonths : $nom,
